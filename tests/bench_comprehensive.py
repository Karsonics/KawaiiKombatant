...
try:
    ...
    print(
        "[WARNING] 'psutil' not installed. Memory monitoring disabled. Run: pip install psutil"
    )
    psutil = None
...
def parse_args():
    ...
    parser.add_argument("--ref_lang", default="en", help="Reference audio language")
    parser.add_argument(
    ...
    parser.add_argument(
        "--runs", default=5, type=int, help="Number of runs per test case"
    )
    parser.add_argument(
    ...
    parser.add_argument("--skip_warmup", action="store_true", help="Skip warm-up run")
    parser.add_argument(
        "--skip_batch", action="store_true", help="Skip batch processing tests"
    )
    parser.add_argument(
        "--skip_concurrency", action="store_true", help="Skip concurrency tests"
    )
    parser.add_argument("--skip_stress", action="store_true", help="Skip stress tests")
    parser.add_argument(
        "--compare", type=str, default=None, help="Compare with previous benchmark JSON"
    )
    parser.add_argument(
        "--text_split_methods",
        nargs="+",
    ...
    parser.add_argument(
        "--languages", nargs="+", default=["en", "zh", "ja"], help="Languages to test"
    )
    parser.add_argument(
        "--export_json", action="store_true", default=True, help="Export JSON results"
    )
    parser.add_argument(
        "--export_csv", action="store_true", default=True, help="Export CSV results"
    )
    parser.add_argument(
        "--save_audio", action="store_true", help="Save sample audio outputs"
    )
...
class GPTSoVITSBenchmark:
    ...
    def run(self):
        ...
        print(
            f"Languages: {self.args.ref_lang} -> {self.args.text_lang or self.args.ref_lang}"
        )
        ...
        print(
            f"Success rate: {(self.total_requests - self.failed_requests)/self.total_requests*100:.1f}%"
        )
    ...
    def _warmup(self):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                print(
                    f"  Audio: {audio_info['duration']:.2f}s, {audio_info['sample_rate']}Hz"
                )
            else:
    ...
    def _synthesize(self, text: str, **kwargs) -> tuple:
        ...
        t0 = time.perf_counter()
        r = requests.post(
            f"{self.base_url}/tts", json=payload, timeout=kwargs.get("timeout", 300)
        )
        elapsed = time.perf_counter() - t0
    ...
    def _run_category_tests(self):
        ...
        total_tests = (
            sum(len(sentences) for sentences in SENTENCE_CATALOG.values())
            * self.args.runs
        )
        completed = 0
        ...
        for category, sentences in SENTENCE_CATALOG.items():
            ...
            for sentence_idx, sentence in enumerate(sentences):
                ...
                if result.successful_runs > 0:
                    print(
                        f"    Avg: {result.avg_time:.2f}s | RTF: {result.rtf:.2f}x | "
    ...
    def _test_text_split_methods(self):
        ...
        for method in self.args.text_split_methods:
            ...
            for run in range(self.args.runs):
                try:
                    data, elapsed = self._synthesize(
                        test_text, text_split_method=method
                    )
        ...
        for batch_size in self.args.batch_sizes:
            ...
            for run in range(self.args.runs):
                ...
                payload = self._create_payload(
                    "|||".join(sentences), batch_size=batch_size
                )

                t0 = time.perf_counter()
                try:
                    _ = requests.post(f"{self.base_url}/tts", json=payload, timeout=300)
                    elapsed = time.perf_counter() - t0
                    times.append(elapsed)
                    print(
                        f"  Run {run+1}: {elapsed:.2f}s ({elapsed/batch_size:.2f}s per item)"
                    )
                except Exception as e:
                    print(f"  Run {run+1}: FAILED - {e}")
        ...
        while time.time() - start < max_duration:
            ...
            try:
                ...
                if success_count % 10 == 0:
                    print(
                        f"  Progress: {success_count} successful, {failure_count} failed"
                    )
            except Exception:
                failure_count += 1
                if failure_count >= 5:
    ...
    def _print_summary_table(self):
        ...
        for category, results in by_category.items():
            ...
            if all_times:
                ...
                print(
                    f"{category:<15} {len(all_times):>6} {avg_time:>9.2f}s {avg_rtf:>7.2f}x "
        ...
        if self.concurrency_results:
            print("\nConcurrency Results:")
            print(
                f"{'Workers':>10} {'Time':>10} {'Success':>10} {'Failed':>10} {'Throughput':>12}"
            )
            print("-" * 60)
            for cr in self.concurrency_results:
                print(
                    f"{cr.num_workers:>10} {cr.total_time:>9.2f}s {cr.successful:>10} "
    ...
    def _compare_results(self):
        ...
        prev_results = {
            r["category"]: r for r in prev_report.get("category_results", [])
        }
        ...
        for category in set(list(prev_results.keys()) + list(curr_results.keys())):
            ...
            if prev and curr:
                ...
                print(
                    f"{category:<15} {prev_rtf:>11.2f}x {curr_rtf:>11.2f}x {change:>+11.1f}%"
                )
